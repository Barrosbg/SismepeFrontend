...
const CadastraPesquisaSatisfacaoPage = (props) => {
    ...
    useEffect(async () => {
        ...
        let cd = 2
        handleLimparcampos()
        setCodigoAtendimento(cd++);
        props.fetchBackdrop('BACKDROP_FETCHED', true);
        let pesquisa = await buscaPesquisaExpirada(cd);

        if (!pesquisa?.data) {
            let pesquisaResp = await buscaPesquisaRespondida(cd);
            if (!pesquisaResp.data) {
                setPesquisaLiberada(true)
                setCodigoAtendimento(3556)
            } else {
                setPesquisaLiberada(false)
                setSituacaoPesquisa('Respondida')
                setExibirModal(true)

            }
        } else {
            setPesquisaLiberada(false)
            setSituacaoPesquisa('Expirada')
            setExibirModal(true)
            setExibirModal(true)
