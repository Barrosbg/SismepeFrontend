...
import { AtualizarLicenca } from '../../actions/jms/actions'
...
import { cadastrarLicenca } from '../../actions/jms/actions'
...
const INITIAL_STATE = {
    ...
    fecharVisivel: false,
    omeId: null,
    textFieldTroca: false,
    hidden: false,
    omeCadastro: null,
    cadastroSucess: false,
    visivel: false,
    habilitaCompo: false,
    page: 0,
    licencaId: null,
...
class manterEGenreicar extends Component {
    ...
    render() {
        const { ome, textFieldTroca, habilitaCompo } = this.state
        return (
            <Container >
                ...
                <Card spacing={2} display="flex" alignItems="center">
                    <CardHeader
                        title="Licença/Dispensa"
                    />
                    <Grid container spacing={1} display="flex" >
                        <Grid item xs={12} sm={6} display="flex" >
                            <TextFieldAutocomplete id="paciente" value={this.state.paciente} label="Paciente" actionFilter={filtrarPessoa} actionChangeOption={this.changePaciente} getOptionLabel={(option) => option.matricula + '-' + option.nome} filterOptions={(options, object) => options.filter((item) => item.nome.toUpperCase().includes(object.inputValue.toString().toUpperCase()) || item.matricula.toString().includes(object.inputValue))} />
                        </Grid>
                        <div className="Spacer"></div>
